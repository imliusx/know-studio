...
- 未显式指定知识库时，可依据当前用户可读知识库的名称与描述进行高置信度定向路由；无法可靠判断时回退授权范围并继续证据校验，不增加额外模型调用延迟。
- 检索链路追踪必须记录路由策略、置信度、证据等级和拒答结果，不记录原问题或文档内容；离线评测支持预期拒答样本并独立计算拒答准确率。
...
- [x] 文档管理总览展示所有可管理知识库的文档，详情页按路由知识库加载，不受全局当前知识库误选影响。
