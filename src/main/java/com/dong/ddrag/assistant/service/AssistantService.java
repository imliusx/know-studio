...
/**
 * Assistant 编排层（对应走读指南「链路 C」）。把"用户消息落库 → 运行 Agent → 助手消息落库"串起来。
 *
 * <p>两个入口：
 * <ul>
 *   <li>{@link #chat}：一次性返回完整回复</li>
 *   <li>{@code streamChat}（在 Controller 里）：SSE 流式，逐字推送</li>
 * </ul>
 * 核心委托给 {@link AssistantAgentFacade} 执行 Agent 推理（含工具调用与记忆重建）。
 * 注意顺序：先落用户消息 → 再跑 Agent（这样记忆 Hook 能基于最新会话状态重建上下文）。
 */
@Service
