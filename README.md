...
如果你正在找一个能写进简历、能讲清架构、能覆盖 Spring AI / Spring AI Alibaba 技术点的项目，know-studio 值得 Star。
...
know-studio 就是围绕这 4 个问题设计的。它把 RAG 从“能跑 demo”推进到“能解释工程边界”的状态。
...
know-studio 中 Spring AI 不是只用一个聊天接口，而是覆盖了多个关键抽象：
...
很多 AI 项目只能展示“我调了模型接口”。know-studio 更适合面试，是因为它能回答更深的问题：
...
当前项目的单机 Docker 启动不依赖 `.env`，需要直接修改配置文件：

- [src/main/resources/application-dev.yml](E:\AICode\know-studio\src\main\resources\application-dev.yml:19)
  将 `spring.ai.dashscope.api-key` 改成你自己的 DashScope API Key。
- [docker-compose.yml](E:\AICode\know-studio\docker-compose.yml:97)
  将 `backend.environment.DASHSCOPE_API_KEY` 改成同一个 DashScope API Key。

如果你还需要调整本机端口或容器访问地址，也需要同步检查：

- [src/main/resources/application-dev.yml](E:\AICode\know-studio\src\main\resources\application-dev.yml:9)
  本地开发直连数据库、Ollama、Elasticsearch、MinIO 的地址。
- [docker-compose.yml](E:\AICode\know-studio\docker-compose.yml:98)
  容器内 backend 连接 PostgreSQL、MinIO、Ollama、Elasticsearch 的地址。

当前仓库默认把 `KNOW_STUDIO_JWT_SECRET` 也写在 `docker-compose.yml` 里，单机启动时无需额外准备环境变量。

### 2. 单机 Docker 启动

确认上面的两个 `DASHSCOPE_API_KEY` 都已经填写后，再执行：
